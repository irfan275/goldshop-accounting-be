...
const { Shop,  AdjustmentLedger } = require("../model");
...
const { getPurchaseLedgerStatement, getAdjustmentLedgerStatement } = require("../services/historyService");
...
const createLedger= async (req, res) => {
  try {
    ...
    const { date, name, description, entries, shop,custId } = req.body;
    
        // 1. Create ledger
        const ledger = await AdjustmentLedger({
          date,
          name,
          custId,
          description,
...
const updateLedger = async (req, res) => {
  ...
  try {
    ...
    let oldLedger = await AdjustmentLedger.findOne(query).lean();
    ...
    const ledger = await AdjustmentLedger.findByIdAndUpdate(
      req.params.id,
      updatedData,
      { new: true }
    );
...
const getAllLedger = async (req, res) => {
  try {
    ...
    const data = await  getAdjustmentLedgerStatement(req);
    return SUCCESS(res, data);
  } catch (e) {
    console.log(e);
    return ERROR(res, StatusCode.SERVER_ERROR, Messages.SERVER_ERROR);
...
const getLedgerById = async (req, res) => {
  try {
    ...
    const ledger = await AdjustmentLedger.findOne(query)
      .lean();
    if (!ledger) {
      return ERROR(res, StatusCode.NOT_FOUND, Messages.LEDGER_NOT_FOUND);
    }
...
const deleteLedger = async (req, res) => {
  try {
    ...
    const data = await AdjustmentLedger.findByIdAndDelete(req.params.id);
    if (!data) {
      return ERROR(res, StatusCode.NOT_FOUND, Messages.LEDGER_NOT_FOUND);
    }
...
const getInvoiceNumberForLedger = async (req, res) => {
  ...
  try {
    ...
    let seqName = `AD-LEG-${shop.shortName}`;
