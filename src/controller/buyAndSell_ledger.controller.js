...
const { Customer, Shop, PurchaseLedger, Ledger, BuyAndSellLedger } = require("../model");
...
const { getPurchaseLedgerStatement, getBuyAndSellLedgerStatement } = require("../services/historyService");
...
const createLedger= async (req, res) => {
  try {
    ...
    const { date, name, description, entries, shop,isBooking,custId } = req.body;
        ...
        // 1. Create ledger
        const ledger = await BuyAndSellLedger({
          date,
          name,
          custId,
          description,
...
const updateLedger = async (req, res) => {
  ...
  try {
    ...
    let oldLedger = await BuyAndSellLedger.findOne(query).lean();
    ...
    const ledger = await BuyAndSellLedger.findByIdAndUpdate(
      req.params.id,
      updatedData,
      { new: true }
    );
...
const getAllLedger = async (req, res) => {
  try {
    ...
    const data = await  getBuyAndSellLedgerStatement(req);
    return SUCCESS(res, data);
  } catch (e) {
    console.log(e);
    return ERROR(res, StatusCode.SERVER_ERROR, Messages.SERVER_ERROR);
...
const getLedgerById = async (req, res) => {
  try {
    ...
    const ledger = await BuyAndSellLedger.findOne(query)
      .lean();
    if (!ledger) {
      return ERROR(res, StatusCode.NOT_FOUND, Messages.LEDGER_NOT_FOUND);
    }
...
const deleteLedger = async (req, res) => {
  try {
    ...
    const data = await BuyAndSellLedger.findByIdAndDelete(req.params.id);
    if (!data) {
      return ERROR(res, StatusCode.NOT_FOUND, Messages.LEDGER_NOT_FOUND);
    }
...
const createBuyAndSellLedgerFromId = async(req, res) => {
  try {
    ...
    const ledger = await Ledger.findById(ledgerId).lean();
      ...
      const purchase = new BuyAndSellLedger(data);
